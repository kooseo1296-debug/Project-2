 ...
 * ------------------------------------
 *
 * Conv1 input (HW-friendly preprocessing experiment):
 *   uint8 RGB
 *   -> rounded per-image integer channel mean
 *   -> integer subtraction
 *   -> signed INT8 saturation
 *   -> inv(input_scale) = 1.0 in Q16 metadata
 *
...
/*
 * HW-friendly input preprocessing experiment.
 *
 * Each RGB channel has exactly 32*32 = 1024 pixels.
 *
 *   sum[c]  = sum of 1024 unsigned pixels
 *   mean[c] = round(sum[c] / 1024)
 *           = (sum[c] >> 10) + bit[9]
 *
 * Then each input sample is centered and clipped directly to signed INT8:
 *
 *   q = saturate_s8(pixel - mean[c])
 *
 * No /255, dataset mean/std normalization, floating-point arithmetic,
 * max-abs search, or input requantization is used.
 *
 * Because q is treated as an integer tensor with scale = 1.0, the reciprocal
 * input scale carried by the existing integer-only bias/scale path is Q16 1.0:
 *
 *   inv_x_scale_q16 = 1 << 16
 */
static u32 preprocess_image_mean_int8_q16(
    const u8 *img,
    s8 *q,
    int verbose
)
{
    ...
    /* First pass: channel sums and rounded integer means. */
    for (c = 0U; c < 3U; c++) {
        const u8 *p = img + c * 1024U;
        ...
        if (mean[c] > 255U) mean[c] = 255U;
    }

    /* Second pass: integer centering and signed INT8 saturation. */
    for (c = 0U; c < 3U; c++) {
        const u8 *p = img + c * 1024U;
        s8 *dst = q + c * 1024U;

        for (hw = 0U; hw < 1024U; hw++) {
            s32 centered = (s32)p[hw] - (s32)mean[c];
    ...
    if (verbose) {
        xil_printf("[input] image mean R=%d G=%d B=%d\r\n",
                   (int)mean[0],
                   (int)mean[1],
                   (int)mean[2]);
        xil_printf("[input] integer mean subtraction + signed INT8 clipping\r\n");
        xil_printf("[input] inv_scale_q16 = 1.0\r\n");
    }
...
)
{
    ...
    /*
     * HW-friendly preprocessing experiment:
     *
     * u8 RGB
     *   -> rounded per-image integer channel mean
     *   -> integer subtraction
     ...
     *   -> Conv1
     *
     * No /255, fixed dataset mean/std normalization, floating point,
     * input max search, or input requantization is used.
     */
    inv_scale_a_q16 = preprocess_image_mean_int8_q16(
        img_chw,
        g_act_a,
        verbose
    );
