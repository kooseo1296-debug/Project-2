...
module systolic (
    ...
    wire [`PE_COL*`PE_ROW*`BIT_VALID-1:0] En_ReLU_In;
    wire [`PE_COL*`PE_ROW*`BIT_VALID-1:0] En_ReLU_Out;
    ...
    generate
        for (i=0; i<`PE_COL; i=i+1) begin : loop_pe_col
            ...
            // Bottom-row outputs
            assign o_Psum_Out[`BIT_PSUM*i +: `BIT_PSUM] =
                Psum_Out[`BIT_PSUM*i + `BIT_PSUM*`PE_COL*(`PE_ROW-1) +: `BIT_PSUM];

            assign o_Addr_P_Out[`ADDR_PSRAM*i +: `ADDR_PSRAM] =
                Addr_P_Out[`ADDR_PSRAM*i + `ADDR_PSRAM*`PE_COL*(`PE_ROW-1) +: `ADDR_PSRAM];

            assign o_Valid_P_Out[`BIT_VALID*i +: `BIT_VALID] =
                Valid_P_Out[`BIT_VALID*i + `BIT_VALID*`PE_COL*(`PE_ROW-1) +: `BIT_VALID];

            assign o_En_ReLU_Out[`BIT_VALID*i +: `BIT_VALID] =
                En_ReLU_Out[`BIT_VALID*i + `BIT_VALID*`PE_COL*(`PE_ROW-1) +: `BIT_VALID];
            ...
            for (j=0; j<`PE_ROW; j=j+1) begin : loop_pe_row
                ...
                pe pe (
                    ...
                    .En_ReLU_In(En_ReLU_In[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID]),
                    .En_ReLU_Out(En_ReLU_Out[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID])
                );


                // Vertical propagation
                if (j > 0) begin
                    assign Data_W_In[`BIT_DATA*i + `BIT_DATA*`PE_COL*j +: `BIT_DATA] =
                        Data_W_Out[`BIT_DATA*i + `BIT_DATA*`PE_COL*(j-1) +: `BIT_DATA];

                    assign EN_W_In[i + `PE_COL*j] =
                        EN_W_Out[i + `PE_COL*(j-1)];

                    assign EN_ID_In[`BIT_ROW_ID*i + `BIT_ROW_ID*`PE_COL*j +: `BIT_ROW_ID] =
                        EN_ID_Out[`BIT_ROW_ID*i + `BIT_ROW_ID*`PE_COL*(j-1) +: `BIT_ROW_ID];

                    assign Psum_In[`BIT_PSUM*i + `BIT_PSUM*`PE_COL*j +: `BIT_PSUM] =
                        Psum_Out[`BIT_PSUM*i + `BIT_PSUM*`PE_COL*(j-1) +: `BIT_PSUM];

                    assign Addr_P_In[`ADDR_PSRAM*i + `ADDR_PSRAM*`PE_COL*j +: `ADDR_PSRAM] =
                        Addr_P_Out[`ADDR_PSRAM*i + `ADDR_PSRAM*`PE_COL*(j-1) +: `ADDR_PSRAM];

                    assign Valid_P_In[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID] =
                        Valid_P_Out[`BIT_VALID*i + `BIT_VALID*`PE_COL*(j-1) +: `BIT_VALID];

                    assign En_ReLU_In[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID] =
                        En_ReLU_Out[`BIT_VALID*i + `BIT_VALID*`PE_COL*(j-1) +: `BIT_VALID];
                end

                // Top-row inputs
                else begin
                    assign Data_W_In[`BIT_DATA*i + `BIT_DATA*`PE_COL*j +: `BIT_DATA] =
                        i_Data_W_In[`BIT_DATA*i +: `BIT_DATA];
                    ...
                    assign Psum_In[`BIT_PSUM*i + `BIT_PSUM*`PE_COL*j +: `BIT_PSUM] =
                        i_Psum_In[`BIT_PSUM*i +: `BIT_PSUM];

                    assign Addr_P_In[`ADDR_PSRAM*i + `ADDR_PSRAM*`PE_COL*j +: `ADDR_PSRAM] =
                        i_Addr_P_In[`ADDR_PSRAM*i +: `ADDR_PSRAM];

                    assign Valid_P_In[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID] =
                        i_Valid_P_In[`BIT_VALID*i +: `BIT_VALID];

                    assign En_ReLU_In[`BIT_VALID*i + `BIT_VALID*`PE_COL*j +: `BIT_VALID] =
                        i_En_ReLU_In[`BIT_VALID*i +: `BIT_VALID];
                end

                // Horizontal activation propagation
                if (i > 0) begin
                    assign Data_I_In[`BIT_DATA*i + `BIT_DATA*`PE_COL*j +: `BIT_DATA] =
                        Data_I_Out[`BIT_DATA*(i-1) + `BIT_DATA*`PE_COL*j +: `BIT_DATA];
                end
                else begin
                    assign Data_I_In[`BIT_DATA*i + `BIT_DATA*`PE_COL*j +: `BIT_DATA] =
                        i_Data_I_In[`BIT_DATA*j +: `BIT_DATA];
                end
            end
        end
    endgenerate
